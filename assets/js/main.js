...
$('#CalMat').on('change', function() {
    ...
    $.ajax({
        ...
        error: function(xhr, status) {
            ...
            alert('Disculpe, existió un problema, Favor de notificarlo al área de sistemas  ');
        },
    });
    return false;
});
...
$('#nomcat').on('input', function() {
    ...
    var texto = $('#catedraticos').find('option[value="' + valor + '"]').attr('label');
    if (texto) {
        $('#nomcat').val(texto);
...
$('#nomcatEdit').on('input', function() {
    ...
    var texto = $('#catedraticosEdit').find('option[value="' + valor + '"]').attr('label');
    if (texto) {
...
jQuery(document).on('submit', '#indiceRep', function(event) {
    ...
    if (Doc === "") {
        sendMessageR('Hay algun problema con el docente. Verifiquelo con el Administrador', "");
        $("input").focus();
        return false;
    }
    if (Sem === "") {
        sendMessageR('Hay algun problema con el semestre. Verifiquelo con el Administrador', "");
        $("input").focus();
        return false;
    }
    if (Car === "") {
        sendMessageR('Hay algun problema con la carrera. Verifiquelo con el Administrador', "");
        $("input").focus();
        return false;
    }
    if (Ide === "") {
        sendMessageR('Hay algun problema con el identificador. Verifiquelo con el Administrador', "");
        $("input").focus();
        return false;
    }
    jQuery.ajax({
...
jQuery(document).on('submit', '#form_AluAcept', function(event) {
    event.preventDefault();
    if (!$('#agree').is(':checked')) {
        alert("Deves aceptar los terminos para continuar, de lo contrario no se guardaran tus datos");
        return false;
    }
    jQuery.ajax({
        ...
        .done(function(res) {
            if (!res.error) {
                ...
                var time = 1000;
                delay(time)
                    .then(function() { $('body').loadingModal('color', 'black').loadingModal('text', 'Tus datos se han guardado correctamente,<br> Espera mientras se genera tu documento.').loadingModal('backgroundColor', 'rgb(158, 130, 14)'); return delay(time); });
...
jQuery(document).on('submit', '#formChangePass', function(event) {
    ...
    if ($('#pas1').val() =='' || $('#pas2').val() =='' || $('#pas1').val() !== $('#pas2').val()) {
        sendMessageR('La contraseña es incorrecta en el campo de confirmacion o esta vacio.', "#DA0000");
        return false;
    }
    jQuery.ajax({
        ...
        .done(function(respuesta) {
            if (!respuesta.error) {
                sendMessageR('La contraseña a sido cambiada correctamente proceda con su visita, muchas gracias.', '#00ff00');
...
jQuery(document).on('submit', '#form_editUser', function(event) {
    ...
    if (userT=='Alu' && !$("#form_editUser input[name='radios']:radio").is(':checked')) {
...
$('#saveUserAT').click(function(event) {
    ...
    if (clave === "") {
        alert("El usuario que intentas registrar no se encuentra dentro de los docentes");
        return false;
    }
    ...
    if (correo === "") {
        alert("debes introducir un correo para poder enviarle sus credenciales al usuario")
        return false;
    }
    jQuery.ajax({
...
function AT_Eliminar(id, nom) {
    var opcion = confirm("Desea eliminar el acceso al Area de tutorias  a " + nom);
    if (opcion == true) {
        jQuery.ajax({
...
function myFunctionClose(text) {
    ...
    var time = 5000;
    delay(time)
        .then(function() { $('body').loadingModal('color', 'black').loadingModal('text', text).loadingModal('backgroundColor', 'rgb(158, 130, 14)'); return delay(time); })
